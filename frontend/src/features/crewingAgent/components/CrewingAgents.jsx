...
import {
  getAllCrewingAgentsAsync,
  resetCrewingAgentUpdateStatus,
  selectCrewingAgentUpdateStatus,
  selectCrewingAgents,
  updateCrewingAgentByIdAsync,
} from "../../crewingAgent/CrewingAgentSlice";
  ...
  return (
    <Stack justifyContent={"center"} alignItems={"center"}>
      <Stack
        ...
        component={"form"}
        noValidate
        onSubmit={handleSubmit(handleUpdateCrewingAgent)}
      >
        {crewingAgents.length ? (
          ...
          >
            <Table aria-label="simple table">
              ...
              <TableBody>
                {crewingAgents.length &&
                  crewingAgents.map((crewingAgent, index) => (
                    <TableRow
                      key={crewingAgent._id}
                      sx={{
                    ...
                    >
                      ...
                      <TableCell>
                        {crewingAgent.shortName} <br />
                        {crewingAgent.agentName}
                      </TableCell>
                      <TableCell>
                        {crewingAgent.contactPerson.title}{" "}
                        {crewingAgent.contactPerson.name}
                        <br />
                        {crewingAgent.email}
                        <br />
                        {crewingAgent.address}
                        <br />
                      </TableCell>
                      <TableCell>
                        {editIndex === index ? (
          ...
          <Stack width={is480 ? "auto" : "30rem"} justifyContent={"center"}>
            <Stack rowGap={"1rem"}>
              ...
              <Typography
                ...
                fontWeight={400}
              >
                There are no crewingAgents currently
              </Typography>
            </Stack>
          </Stack>
        )}
